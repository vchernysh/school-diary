...
namespace app\controllers\students\_my_class;
 ...
 */
class RatingController extends StudentCONTROLLER
{
    ...
    public function actionMarks($subject_id, $from = null, $to = null, $only_me = null)
    {
        ...
        if (in_array($subject_id, array_keys($subjects))) {
            ...
            $current_student_name = $students[Yii::$app->user->identity->id];
            unset($students[Yii::$app->user->identity->id]);
            ...
            if (!is_null($only_me) && $only_me == 1) {
                $students = [Yii::$app->user->identity->id => $current_student_name];
            } else {
                $students = [Yii::$app->user->identity->id => $current_student_name] + $students;
            }
