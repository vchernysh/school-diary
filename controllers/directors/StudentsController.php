...
use Yii;
use app\models\{Students, User, Classes, Parents};
...
class StudentsController extends DirectorsController
{
    ...
    public function actionCreate()
    {
        ...
        $classes = [];
        $classes_query = Classes::find()->where(['school_id' => $this->_school->id])->all();

        if ($classes_query) {
        	$i = 0;
        	foreach($classes_query as $class) {
        		$classes[$i]['id'] = $class['id'];
        		$classes[$i]['name'] = $class['name'];
        		$i++;
        	}
