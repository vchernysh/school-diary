...
use app\models\{Marks, ClassTeachersAccess, ClassSubjects, ClassTeachersSubjectsAccess, Classes, Subjects, Students, User};
 ...
 */
class MarksController extends DirectorsController
{
    ...
    public function actionIndex()
    {
        ...
        $model = new DynamicModel(['classes', 'subjects']);
        $model->addRule(['subjects'], 'required', ['message' => 'Необхідно вибрати предмет.'])
            ->addRule(['classes'], 'required', ['message' => 'Необхідно вибрати клас.']);

        $classes_access = ClassTeachersAccess::getClassesByTeachersAccess(Yii::$app->user->identity->id);
        $classes = [];
        if ($classes_access) {
            foreach($classes_access as $key => $value) :
                $classes[$value['class_id']] = $value['name'];
            endforeach;
        }
        ...
        if (Yii::$app->request->post('DynamicModel')) {
            ...
            $class_id = $post['DynamicModel']['classes'];

            $check_class_subjects = ClassSubjects::findOne(['class_id' => $class_id, 'subject_id' => $subject_id]);
            $check_class_teacher_access = ClassTeachersAccess::findOne(['class_id' => $class_id, 'teacher_id' => Yii::$app->user->identity->id]);
            $check_class_teacher_subject_access = ClassTeachersSubjectsAccess::findOne(['class_id' => $class_id, 'subject_id' => $subject_id, 'teacher_id' => Yii::$app->user->identity->id]);

            if ($check_class_subjects && $check_class_teacher_access && $check_class_teacher_subject_access) {

                if ($post['add-new-mark']) {
                    return $this->redirect(['add-new-marks', 'class_id' => $class_id, 'subject_id' => $subject_id]);
                } elseif ($post['get-marks-sheet']) {
                    return $this->redirect(['marks', 'class_id' => $class_id, 'subject_id' => $subject_id]);
                } else {
        ...
        return $this->render('index', compact('model', 'classes'));
    }
        ...
        if (Yii::$app->request->isAjax)
        {
            ...
            if ($post['action'] == 'check_titles') {
                ...
                if ($check) {
                    ...
                    Yii::$app->session->setFlash('error', 'Трапилася помилка, спробуйте ще раз!');
                    return $this->redirect(['add-new-marks', 'class_id' => $class_id, 'subject_id' => $post['subject_id']]);
        ...
        if (Yii::$app->request->isAjax)
        {
            ...
            if ($post['date']) {
                ...
                if ($check_class_subjects && $check_class_teacher_access && $check_class_teacher_subject_access) {
                    ...
                    $students = Students::getStudentsByClass($class_id);
                    $subject = Subjects::findOne($subject_id);
                    $check_columns = [];

                    $marks_of_date = Marks::getMarks($class_id, $subject_id, $search_date);
    ...
    public function actionAddNewMarks($class_id, $subject_id)
    {
        ...
        if ($check_class_subjects && $check_class_teacher_access && $check_class_teacher_subject_access) {
            ...
            $class = Classes::findOne(['id' => $class_id]);
            ...
            if (Yii::$app->request->post() && Yii::$app->request->post('save-marks-submit-button') == 'true') {
                ...
                return $this->redirect(['marks', 'class_id' => $class_id, 'subject_id' => $post['subject_id']]);
            }

            return $this->render('add-new-marks', compact('subject_id', 'subject', 'model', 'students', 'titles', 'marks', 'class_id', 'class'));
        }
    ...
    public function actionMarks($class_id, $subject_id, $from = null, $to = null)
    {
        ...
        if ($check_class_subjects && $check_class_teacher_access && $check_class_teacher_subject_access) {
        	...
        	$classSubjects = Subjects::getClassSubjectsByTeacherAccess($class_id, Yii::$app->user->identity->id);
	        $subjects = ArrayHelper::map($classSubjects, 'id', 'subject_name');

	        if (Yii::$app->request->isAjax)
	        {
	            return $this->redirect(['marks', 'class_id' => $class_id, 'subject_id' => Yii::$app->request->get('subject_id')]);
	        }
	        ...
	        if (in_array($subject_id, array_keys($subjects))) {
                ...
                if (Yii::$app->request->post('filter-search-submit-button') && Yii::$app->request->post('filter-search-submit-button') == 'true')
                {
                    ...
                    if (!empty($date_from) && !empty($date_to)) {
                        ...
                        if ($period >= 0) {
                            return $this->redirect(['marks', 'class_id' => $class_id, 'subject_id' => $subject_id, 'from' => $date_from, 'to' => $date_to]);
                        } else {
                        ...
                        if (!empty($date_from)) {
                            return $this->redirect(['marks', 'class_id' => $class_id, 'subject_id' => $subject_id, 'from' => $date_from]);
                        } elseif (!empty($date_to)) {
                            return $this->redirect(['marks', 'class_id' => $class_id, 'subject_id' => $subject_id, 'to' => $date_to]);
                        }
                    }
	            ...
	            $students = Students::getStudentsByClass($class_id);
	            $marks = Marks::getMarks($class_id, $subject_id, null, $from, $to);
	            ...
	            $skips_was_not = Marks::find()
                    ...
                    ->andWhere(['class_id' => $class_id, 'subject_id' => $subject_id])
                    ->groupBy('student_id')
                    ->asArray()->all();
	            ...
	            return $this->render('marks', compact('subject', 'marks', 'students', 'subjects', 'titles', 'colspan_number', 'student_skips', 'subject_id', 'class_id', 'class'));
    ...
    public function actionGetClassSubjects($id)
    {
        if (Yii::$app->request->isAjax) {

            $check = ClassTeachersAccess::findOne(['class_id' => $id, 'teacher_id' => Yii::$app->user->identity->id]);

            if ($check) {

                $subjects = ClassTeachersAccess::getSubjects($id, Yii::$app->user->identity->id);
                $html = '';
                if ($subjects) {
                    foreach(ArrayHelper::map($subjects, 'id', 'subject_name') as $key => $subject_name) :
                        $html .= '<option value="' . $key . '">' . $subject_name . '</option>';
                    endforeach;
                } else {
                    $html = '<option selected disabled>Немає жодного предмета, по якому Ви можете виставляти оцінки</option>';
                }
