...
use Yii;
    use app\models\{Parents, Students};
 ...
 */
class MyFamilyController extends ParentCONTROLLER
{
    ...
    public function actionIndex()
    {
        ...
        $dataProvider->query->andWhere(['classes.school_id' => $this->_school->id, 'classes.id' => $this->_class->id, 'parents.student_id' => Yii::$app->user->identity->children->user_id])->orderBy(['user.name' => SORT_ASC]);

        $studentDataProvider = new ActiveDataProvider([
            'query' => Students::find()->where(['user_id' => Yii::$app->user->identity->children->user_id]),
        ]);
    ...
    public function actionView($id)
    {
    	...
    	if ($this->findModel($id)->school->id == $this->_school->id && $this->findModel($id)->class->id == $this->_class->id && $this->findModel($id)->student_id == Yii::$app->user->identity->children->user_id) {
