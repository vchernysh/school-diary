    ...
    <p class="class-for-paragraph-links">
        ...
        <?php if (Yii::$app->controller->school__payment_type == 'all' && Yii::$app->user->identity->school->is_test == 'no') { ?>
            <?php if (Yii::$app->controller->school__max_students > Yii::$app->controller->_count_students) { ?>
                <?= Html::a('Додати нового учня', ['create'], ['class' => 'btn btn-success']) ?>
            <?php } ?>
        <?php } else { ?>
            <?= Html::a('Додати нового учня', ['create'], ['class' => 'btn btn-success']) ?>
        <?php } ?>
        ...
        <?= Html::a('Видалити', ['delete', 'id' => $model->user_id], [
            ...
            'data' => [
                'confirm' => 'Ви впевнені, що хочете видалити цього учня? Усі дані стосовно цього учня будуть видалені, включаючи його батьків, які зареєстровані у системі School Diary.',
                'method' => 'post',
            ],
        ]) ?>
    </p>
