...
$this->title =  $class->name . ' - Оцінки - ' . $subject->subject_name;
...
<div class="flex-block marks-flex-block">
	<?= Html::a('Додати нову оцінку', ['add-new-marks', 'class_id' => $class_id, 'subject_id' => $subject_id], ['class' => 'btn btn-success']); ?>
	...
	<?= Html::dropDownList('subject', $subject_id, $subjects, ['class' => 'form-control select-subject-marks', 'onchange' => '
		$.post("/directors/teachers/marks/marks?class_id='.'"+$("#hidden_class_id").val()+"'.'&subject_id='.'"+$(this).val());
	']) ?>
  <?= Html::hiddenInput('class_id', $class_id, ['id' => 'hidden_class_id']); ?>
</div>
<br>
      ...
      <div class="row">
        <div class="col-md-12 flex-block">
          ...
          <?php if (Yii::$app->request->get('from') || Yii::$app->request->get('to')) { ?>
            <?= Html::a('Показати всі оцінки без фільтрів', Url::to(['marks', 'class_id' => $class_id, 'subject_id' => $subject_id]), ['class' => 'btn btn-warning all-marks-without-filters']) ?>
          <?php } ?>
        </div>
