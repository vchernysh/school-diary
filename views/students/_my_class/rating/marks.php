...
$this->params['breadcrumbs'][] = ['label' => 'Мій клас - ' . Yii::$app->user->identity->student_class->name . ' (Успішність <i class="fa fa-line-chart" aria-hidden="true"></i>)', 'url' => ['index']];
$this->params['breadcrumbs'][] = $this->title;

?>
<div class="flex-block marks-flex-block students-view-marks-flex-block-wrap">
	...
	<?= Html::dropDownList('subject', $subject_id, $subjects, ['class' => 'form-control select-subject-marks', 'onchange' => '
		$.post("/students/my-class/rating/marks?subject_id='.'"+$(this).val());
	']) ?>
</div>
...
<?php if ($marks) { ?>
  ...
  <div class="form-group filter-search-marks-wrap-block">
    <form action="" method="post" class="filter-search-marks-wrap-form">
      ...
      <div class="row">
        <div class="col-md-12">
          <?php if (Yii::$app->request->get('only_me') && Yii::$app->request->get('only_me') == 1) { ?>
            <?= Html::a('Показати всі оцінки', Url::to(['marks', 'subject_id' => $subject_id] + $array_of_get_parameters), ['class' => 'btn btn-warning']); ?>
          <?php } else { ?>
            <?= Html::a('Тільки мої оцінки', Url::to(['marks', 'subject_id' => $subject_id, 'only_me' => '1'] + $array_of_get_parameters), ['class' => 'btn btn-warning']); ?>
          <?php } ?>
          <br>
          <br>
        </div>
        <div class="col-md-12 flex-block">
          ...
          <?php if (Yii::$app->request->get('from') || Yii::$app->request->get('to')) { ?>
            ...
            <?php if (Yii::$app->request->get('only_me') && Yii::$app->request->get('only_me') == 1) { ?>
              <?= Html::a('Показати всі оцінки без фільтрів', Url::to(['marks', 'subject_id' => $subject_id, 'only_me' => '1']), ['class' => 'btn btn-warning all-marks-without-filters']) ?>
            <?php } else { ?>
      ...
      <tbody>
        <?php foreach($students as $id => $name) : ?>
          <tr class="my_tr" title="<?= $name ?>">
            <td><span class="student_name_span" data-student-id="undefined"><?= $name ?></span></td>
            <?php $i = 2; foreach($marks as $date => $value) { ?>
